...
class BrandsController extends Controller
{
    ...
    public function edit($id)
    {
        ...
        return view('admin.pages.brands.edit', compact('row'));
    }

    public function update(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'row_id' => 'required|exists:brands,id',
            'title_ar' => 'required',
            'title_en' => 'required',
            'active' => 'required|in:0,1',
...
//            }
//        }
        $row = Brand::whereId($request->row_id)->first();
    ...
    public function delete(Request $request)
    {
        ...
        $row = Brand::where('id', $request->row_id)->first();
//        if (!empty($city->getOriginal('image'))){
//            unlinkFile($city->getOriginal('image'), 'cities');
//        }
        $row->delete();
...
//                if ($auth->can('sliders.update')) {
                $buttons .= '<a href="' . route('admin.brands.edit', [$row->id]) . '" class="btn btn-primary btn-circle btn-sm m-1" title="تعديل">
